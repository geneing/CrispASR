...
static bool fetch_winhttp(const std::string & url,
                          ...
                          bool quiet)
{
    ...
    for (int hop = 0; hop < 10; ++hop) {
        ...
        if (status >= 300 && status < 400) {
            ...
            WinHttpCloseHandle(hConn);
            WinHttpCloseHandle(hSess);
            if (current_url.empty()) return false;
        ...
        if (!quiet)
            fprintf(stderr, "crispasr: WinHTTP: server returned %lu for %s\n",
                    static_cast<unsigned long>(status), current_url.c_str());
        WinHttpCloseHandle(hReq);
        WinHttpCloseHandle(hConn);
        WinHttpCloseHandle(hSess);
        return false;
    ...
    if (!quiet) fprintf(stderr, "crispasr: WinHTTP: too many redirects\n");
    return false;
}
...
// Returns true iff the file was written and is non-empty.
static bool fetch_libcurl(const std::string & url,
                          const std::string & dest,
                          bool quiet)
{
...
std::string dir(const std::string & cache_dir_override) {
    ...
    if (!home || !*home) home = std::getenv("LOCALAPPDATA");
#endif
...
bool fetch(const std::string & url, const std::string & dest, bool quiet) {
    ...
    if (!quiet) fprintf(stderr, "crispasr: downloading via WinHTTP...\n");
    if (fetch_winhttp(url, dest, quiet) && file_present(dest)) return true;
    if (!quiet) fprintf(stderr, "crispasr: WinHTTP failed, falling back...\n");
#endif

#ifdef CRISPASR_USE_LIBCURL
    // ── libcurl API (preferred on POSIX; fallback after WinHTTP on Windows) ──
    if (!quiet) fprintf(stderr, "crispasr: downloading via libcurl...\n");
    if (fetch_libcurl(url, dest, quiet) && file_present(dest)) return true;
    if (!quiet) fprintf(stderr, "crispasr: libcurl failed, falling back to curl CLI...\n");
#endif
