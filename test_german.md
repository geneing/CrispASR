...
Two German clips from [Wikimedia Commons / Audio files in German](https://commons.wikimedia.org/wiki/Category:Audio_files_in_German), spanning <30 s and >30 s. Both runtimes are q4_k, 8-thread CPU.
...
Both converted to 16 kHz mono WAV with `ffmpeg -ar 16000 -ac 1 -c:a pcm_s16le`.
...
### 2. `sarma.wav` — Amardeo Sarma intro
...
3. **Parakeet code-switches into English on German clips with technical vocabulary or proper nouns.** Sarma's recording is classified as German correctly, but the model drops into English on words like "1955", "Indien", "Industrial Forschung", "Technische University". This is **not a chunking issue** — VAD-based segmentation gives the same code-switching.

4. **Parakeet handles short clean German fine** — `jazeschann` (4.8 s phrase) is a perfect tie with Cohere. The code-switching only shows up on longer / vocabulary-heavy clips.
...
After hitting the parakeet language drift above, we ported `nvidia/canary-1b-v2` (the encoder–decoder sister model from the same NeMo team) to ggml in the same fork. Canary takes **explicit `-sl LANG` / `-tl LANG`** task tokens, removing the auto-detect ambiguity entirely.

Re-running the same German clips with `canary-main -sl de -tl de`:
