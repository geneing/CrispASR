...
struct moonshine_context * moonshine_init_with_params(struct moonshine_init_params params) {
    ...
    if (hp.enc_hidden_size == 0 || hp.n_heads == 0 || hp.enc_n_layers == 0 || hp.dec_n_layers == 0) {
        fprintf(stderr, "%s: invalid model hparams (hidden=%u heads=%u enc_layers=%u dec_layers=%u)\n",
                __func__, hp.enc_hidden_size, hp.n_heads, hp.enc_n_layers, hp.dec_n_layers);
        gguf_free(ctx_gguf);
        delete ctx;
        return nullptr;
    }
...
static struct ggml_tensor * build_conv_stem(
        struct ggml_context * ctx0,
    ...
    // Conv1 (no bias) + tanh
    struct ggml_tensor * cur = conv_1d_f32(ctx0, model.enc_conv1_w, audio,
                                           hp.conv1_stride, 0, 1);
    ...
    cur = ggml_mul(ctx0, cur,
                   ggml_reshape_3d(ctx0, model.enc_groupnorm_w, 1, hidden, 1));
    cur = ggml_add(ctx0, cur,
                   ggml_reshape_3d(ctx0, model.enc_groupnorm_b, 1, hidden, 1));

    // Conv2 + bias + GELU
    cur = conv_1d_f32(ctx0, model.enc_conv2_w, cur,
                      hp.conv2_stride, 0, 1);
    cur = ggml_add(ctx0, cur,
                   ggml_reshape_3d(ctx0, model.enc_conv2_b, 1, model.enc_conv2_b->ne[0], 1));
    cur = ggml_gelu_erf(ctx0, cur);

    // Conv3 + bias + GELU
    cur = conv_1d_f32(ctx0, model.enc_conv3_w, cur,
                      hp.conv3_stride, 0, 1);
    cur = ggml_add(ctx0, cur,
                   ggml_reshape_3d(ctx0, model.enc_conv3_b, 1, hidden, 1));
    cur = ggml_gelu_erf(ctx0, cur);
...
static struct ggml_tensor * moonshine_build_encoder(
        struct ggml_context * ctx0,
        const moonshine_model & model,
    ...
    for (uint32_t il = 0; il < hp.enc_n_layers; il++) {
        ...
        Q = ggml_rope_ext(ctx0, Q, pos, nullptr, rotary_dim, 0, 0,
                          rope_theta, 1.0f, 0.0f, 1.0f, 0.0f, 0.0f);
        K = ggml_rope_ext(ctx0, K, pos, nullptr, rotary_dim, 0, 0,
                          rope_theta, 1.0f, 0.0f, 1.0f, 0.0f, 0.0f);
...
static struct ggml_tensor * moonshine_build_decoder_step(
        struct ggml_context * ctx0,
        const moonshine_model & model,
        moonshine_kv_cache & kv_self,
        moonshine_kv_cache & kv_cross,
...
static int moonshine_decode_step(struct moonshine_context * ctx, int32_t token_id,
    ...
    struct ggml_tensor * logits = moonshine_build_decoder_step(
        ctx0, ctx->model, ctx->kv_self, ctx->kv_cross,
        inp_token, inp_pos, ctx->enc_len, cur_pos, graph);
...
const char * moonshine_transcribe(struct moonshine_context * ctx, const float * audio, int n_samples) {
    ...
    if (!moonshine_kv_cache_init(ctx->kv_cross, hp.dec_n_layers, ctx->enc_len,
                                  hp.n_kv_heads, hp.head_dim)) {
        ctx->kv_self.reset();
        return "";
    }
    ...
    ggml_gallocr_t gallocr = ggml_gallocr_new(ggml_backend_get_default_buffer_type(ctx->backend));
    {
        ...
        struct ggml_tensor * plan_logits = moonshine_build_decoder_step(
            ...
            t, p, ctx->enc_len, max_len - 1, plan_graph);
...
void moonshine_print_model_info(struct moonshine_context * ctx) {
    ...
    for (struct ggml_tensor * t = ggml_get_first_tensor(ctx->model.ctx_w);
        ...
        if (t->ne[3] > 1) {
            ...
            snprintf(shape, sizeof(shape), "[%lld]",
                     (long long)t->ne[0]);
        }
